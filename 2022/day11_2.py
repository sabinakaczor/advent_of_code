...
class Item:
    ...
    def add_operation(self, operation: tuple):
        self.operations.append(operation)
        
    def resolve_remainder(self, divisor) -> int:
        [index, value] = self.cached_results[divisor] if divisor in self.cached_results else (0, self.value % divisor)
            
        for operation in self.operations[index:]:
            index += 1
            [operator, factor] = operation
            if operator == '*':
        ...
        self.cached_results[divisor] = (index, value)
...
class KeepAwayGame:
    ...
    def count_monkey_business(self) -> int:
        activity = {i: p.inspected_items_count for i, p in enumerate(self.players)}
...
class Monkey:
    ...
    def play(self, game: KeepAwayGame, starting_items: list, operation: str, test: tuple) -> None:
        self.game = game
        self.items = [Item(value) for value in starting_items]
    ...
    def resolve_operation(self, operation: str):
        ...
        if self.operator == '*' and self.factor == 'old':
            self.operator = '**'
            self.factor = 2
        else:
    ...
    def inspect_current_item(self):
        ...
        self.items[0].add_operation((self.operator, self.factor))
        
        self.inspected_items_count += 1
        
    def test_current_item(self) -> int:
        remainder = self.items[0].resolve_remainder(self.divisor)
        i = 0 if remainder == 0 else 1
